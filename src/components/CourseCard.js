...
export default function CourseCard({ course }) {
  ...
  return (
    <div
      ...
      onClick={() => !confirmDelete && router.push(`/course/${course.id}`)}
      onMouseEnter={e => {
    ...
    >
      ...
      <div style={{ position: 'relative', aspectRatio: '16/9', overflow: 'hidden', background: '#111' }}>
        ...
        {pct > 0 && (
          <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 3, background: 'rgba(0,0,0,0.5)' }}>
            <div style={{ height: '100%', width: `${pct}%`, background: 'var(--accent)', transition: 'width 0.6s ease' }} />
          </div>
        )}
        {pct === 100 && (
          <div style={{
      ...
      <div style={{ padding: '14px 16px 16px' }}>
        <h3 style={{
          ...
          fontSize: 16, lineHeight: 1.3, marginBottom: 4,
      ...
      {!confirmDelete ? (
        ...
        >
          <span style={{ color: '#fff', fontSize: 12 }}>Delete this course?</span>
