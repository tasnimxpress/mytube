# MyTube · FocusLearn
---

MyTube turns any public YouTube playlist into a clean, Udemy-style learning experience. 

## Features

- **Distraction-free player** — YouTube embedded without sidebar recommendations or related videos
- **Course progress tracking** — check off videos as you watch, see your % completion
- **Resume where you left off** — automatically returns to your last watched video
- **Saved locally** — all data lives in a file on your machine, no account needed
- **Export / Import** — back up or transfer your progress anytime as a JSON file

## Tech Stack

Next.js · Tailwind CSS · Piped API · File System Access API · Vercel

---
